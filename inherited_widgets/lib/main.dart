...
class NameInheritedWidget extends InheritedWidget {
  const NameInheritedWidget({Key key, this.name, Widget child})
...
class _NamePageState extends State<NamePage> {
  ...
  Widget build(BuildContext context) {
    return new NameInheritedWidget(
        name: name, child: const IntermediateWidget());
  }
}
...
class IntermediateWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return new Center(
        child: new Padding(
            padding: new EdgeInsets.all(10.0), child: const NameWidget()));
  }
}
